...
\begin{equation}
q(z|x, \phi) = \normalpdf{z}{\mu(x, \phi)}{ \Sigma(x, \phi)}
\end{equation}
...
\begin{equation}
\kldiv{q_{|x,\phi}}{p_{|x, \theta}} = \int \dd z\, q(z|x, \phi)\
\log \frac{q(z|x, \phi) p(x|\theta)}{p(x, z| \theta)}
\end{equation}

The KL divergence still depends on the intractable latent-marginal posterior
$$p(x|\theta)$$, but only through a constant that does not depend on $$q$$.
Therefore, the following objective is both tractable, and entirely equivalent
to the KL divergence for the purpose of obtaining the approximation $$q$$:
\begin{equation}
L(q, p, x, \theta, \phi) = \int \dd z\, q(z|x, \phi)\ \log \frac{q(z|x, \phi)} {p(x, z| \theta)}
\end{equation}

\begin{equation}
L(q, p, x, \theta, \phi) = \kldiv{q_{|x,\phi}}{p_{|x, \theta}} - \log p(x|\theta)
\end{equation}
